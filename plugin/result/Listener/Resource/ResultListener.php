...
namespace Claroline\ResultBundle\Listener\Resource;
...
class ResultListener
{
    ...
    /**
     * Loads a Result resource.
     *
     * @DI\Observe("resource.claroline_result.load")
     *
    ...
    {
        $event->setData([
            'result' => $this->serializer->serialize($event->getResource()),
        ]);
    ...
    /**
     * Copies a result resource.
     *
     * @DI\Observe("copy_claroline_result")
     *
    ...
    /**
     * Removes result notifications when a user is deleted.
     *
    ...
    public function onDeleteUser(DeleteUserEvent $event)
    {
        ...
        $notificationRepo = $this->om->getRepository('ClarolineResultBundle:Notification');
    ...
    /**
     * Creates evaluation for result resource.
     *
     * @DI\Observe("generate_resource_user_evaluation_claroline_result")
     *
    ...
    public function onGenerateResourceTracking(GenericDataEvent $event)
    {
        ...
        $logs = $this->evaluationManager->getLogsForResourceTracking(
            $node,
            $user,
            ['resource-read', 'resource-claroline_result-create_message'],
            $startDate
        );
            ...
            foreach ($logs as $log) {
                switch ($log->getAction()) {
                    case 'resource-read':
                        ...
                        break;
                    case 'resource-claroline_result-create_message':
                        ++$nbAttempts;
