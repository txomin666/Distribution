...
namespace Innova\AudioReactBundle\Controller;

use Claroline\CoreBundle\Entity\Workspace\Workspace;
use Innova\AudioReactBundle\Entity\MediaResource;
use Innova\AudioReactBundle\Entity\Options;
 ...
 */
class AudioReactController extends Controller
{
    /**
     * display a media resource.
     *
     * @Route("/open/{id}", requirements={"id" = "\d+"}, name="innova_audio_react_open")
     * @Method("GET")
     */
    public function openAction(Workspace $workspace, MediaResource $mr)
    {
        ...
        return $this->render('InnovaAudioReactBundle:MediaResource:players.html.twig', [
                  '_resource' => $mr
              ]
      );
    }
     ...
     *
     * @Route("/edit/{id}", requirements={"id" = "\d+"}, name="innova_audio_react_administrate")
     * @Method("GET")
     */
    public function administrateAction(Workspace $workspace, MediaResource $mr)
    {
        ...
        return $this->render('InnovaAudioReactBundle:MediaResource:administrate.html.twig', [
                    '_resource' => $mr,
          ]
        );
    }
    ...
    public function save(Workspace $workspace, MediaResource $resource)
    {
        ...
        $this->get('innova_audio_react.manager.media_resource_options')->update($resource->getOptions(), $data);
        $this->get('innova_audio_react.manager.media_resource_region')->updateRegions($resource, $data);
    ...
    {
        $fileUrl = $this->get('innova_audio_react.manager.media_resource_media')->getAudioMediaUrlForAjax($mr);
    ...
    {
        $data = $this->container->get('request')->request->all();
        $zipData = $this->get('innova_audio_react.manager.media_resource')->exportToZip($resource, $data);
