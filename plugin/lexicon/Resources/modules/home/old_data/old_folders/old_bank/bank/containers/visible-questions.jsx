  ...
  return {
    ...
    onShare: (items) => {
      dispatch(showModal(MODAL_SHARE, {
        title: transChoice('Partager', items.length, {count: items.length}, 'claroline_lexicon'),
        fadeModal: fadeModal,
    ...
    onDelete: (items) => {
      dispatch(showModal(MODAL_DELETE_CONFIRM, {
        title: transChoice('Supprimer', items.length, {count: items.length}, 'claroline_lexicon'),
        question: trans('remove_question_confirm_message'),
